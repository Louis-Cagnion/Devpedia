...
Only the binary `serveur` is copied from the `builder` step to the final image—the Go compiler (several hundred MB) is never included in the delivered image.
