...
```bash
echo "Contents" | git hash-object -w --stdin   # Creates a blob and displays its hash
git cat-file -p a3f9c1d                        # Displays the decompressed contents of an object
...
## Rewrite the entire history: purge one file from each commit
...
- `git filter-branch` It automatically saves a backup copy in `refs/original/` (which should be explicitly deleted—`git update-ref -d refs/original/refs/heads/main`—once you are certain you no longer need it).
...
| **Pitfalls to Avoid** | `filter-branch` without first saving—an error in the filter is just as irreversible as the operation itself. |
