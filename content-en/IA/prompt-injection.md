...
> **Note:** the chatbot chapter already warns against the first of these cases; see *"Never put a secret in the system prompt"* in [Building a Chatbot](/?c=ia&p=chatbot): if the confidential instruction isn't there, the leak costs the attacker who obtains it nothing.
...
Against a chatbot that only responds in text, a successful injection at worst makes the model say something inappropriate or leaks a system prompt. Against an [agent](/?c=ia&p=agents) that can call tools (send an email, run a query, modify a database), the same injection can make the model **act**: an instruction hidden in a document consulted by the agent can make it run a tool that no one legitimately requested: exfiltrating data to an external address, deleting a resource, approving a transaction. This is exactly the *"irreversible actions decided by a fallible system"* risk already covered in [Agents](/?c=ia&p=agents): prompt injection is one of the concrete ways that abstract risk actually gets triggered in practice.
...
|---|---|---|
| Strict instruction/data delimitation | Clearly separate, with tags or triple quotes, what is an instruction from what is data to process (see [Structuring the Prompt](/?c=ia&p=prompt-engineering)) | Reduces ambiguity, doesn't eliminate it: a model remains a probabilistic system, not a strict parser |
| Input and output filtering | Detect, before sending to the model or before displaying the response, known patterns of instruction attempts (see [LLM Monitoring and Operations](/?c=ia&p=gestion-dun-llm)) | The classic arms race: a pattern filtered today lets through a rephrasing not yet catalogued tomorrow |
...
| Human confirmation before an irreversible action | A human approves before an action with real consequences goes out (see [Agents](/?c=ia&p=agents)) | Costs fluidity; ineffective if the confirmation itself becomes an unread reflex ("click without looking") |
