...
The "fully connected" network described in the [chapter on fundamentals](/?c=ia&p=reseaux-de-neurones) (where each neuron is connected to all neurons in the next layer) is not the only way to organize layers. Depending on the type of data being processed (image, sequence, text), certain architectures are much more efficient. This chapter presents the three most influential families.
...
"The cat sleeping on the couch is black."
                                    ^
                     l'attention permet à "is black" de se relier directement à "chat",
...
- This architecture forms the basis of modern large language models (LLMs) (see [NLP and LLM](/?c=ia&p=nlp-et-llm)).
...
>
> **Best practice:** For very long sequences, check the context limits of the model being used (see [LLM in Production](/?c=ia&p=llm-en-production)) rather than assuming that a Transformer can handle any length without additional cost.
...
| **Transform** | Sequences, text, and an increasing number of images as well | Scalable to parallel processing; captures long-range dependencies via attention | High memory and computational cost for very long sequences |

See also [NLP and LLM](/?c=ia&p=nlp-et-llm) for the application of the Transformer architecture to natural language processing.
...
| **Available Tools** | Deep learning libraries provide ready-to-use layers for each architecture (see [PyTorch](/?c=ia&p=deep-learning-pytorch)). |
