...
A **CRNN** (*Convolutional Recurrent Neural Network*) combines the two [architectures seen earlier](/?c=ia&p=architectures-cnn-rnn-transformers) rather than inventing a new one:
...
As with plain text (see [NLP and LLM](/?c=ia&p=nlp-et-llm)), a CRNN's RNN can be replaced with an **attention** mechanism. A Transformer architecture for OCR (e.g. [TrOCR](https://arxiv.org/abs/2109.10282)) is made of two blocks:
...
This architecture no longer depends on CTC: the decoder directly generates a sequence of characters, the way an LLM generates a sequence of words (see [Natural Language Processing (NLP) and Large Language Models (LLMs)](/?c=ia&p=nlp-et-llm)), without a CRNN's column-by-column alignment constraints.
...
See also [Structured OCR and layout analysis](/?c=traitement-de-documents&p=ocr-structure) for the step that uses this recognized text (placing it back into a page structure), and [Architectures: CNNs, RNNs, and Transformers](/?c=ia&p=architectures-cnn-rnn-transformers) for the details of the building blocks (CNN, RNN, attention) reused here.
...
| **Tools you can use** | Tesseract (the historic engine, classic OCR then LSTM+CTC in its recent versions), CRNN+CTC or Transformer models trainable with [PyTorch](/?c=ia&p=deep-learning-pytorch). |
