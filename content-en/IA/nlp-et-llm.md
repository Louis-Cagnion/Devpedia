...
The attention mechanism (see [Architectures — CNN, RNN, and Transformers](/?c=ia&p=architectures-cnn-rnn-transformers)) lets each token "look at" the other tokens in the sequence to adjust its own representation based on context:
...
See also [Architectures — CNN, RNN, and Transformers](/?c=ia&p=architectures-cnn-rnn-transformers) (the underlying attention mechanism), [Deep Learning with PyTorch](/?c=ia&p=deep-learning-pytorch) (how such a model is concretely trained, at a much smaller scale in that chapter's examples) and [Prompt Engineering](/?c=ia&p=prompt-engineering) (how to concretely phrase a good prompt).
