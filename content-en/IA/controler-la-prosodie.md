...
[Tacotron](/?c=ia&p=synthese-classique-vs-deep-learning) only controls prosody **implicitly**: the model has learned, from training examples, a plausible prosody for a given text, with no explicit model parameter representing "pitch" or "duration" separately. More recent architectures add **explicit** control:
...
The browser's [Web Speech API](/?c=ia&p=synthese-classique-vs-deep-learning), far simpler than a neural model with explicit control, already exposes the three components above, in a form reduced to a single global setting per sentence rather than a detailed curve:
...
See also [Modern Synthesis Models](/?c=ia&p=modeles-modernes-synthese) for architectures that go further than this basic explicit control.
