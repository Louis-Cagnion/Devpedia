...
A [neural network](/?c=ia&s=fondamentaux-du-deep-learning&p=reseaux-de-neurones) works with numbers, never directly with text. Natural Language Processing (NLP) groups the techniques that convert text into usable numerical representations, the essential preliminary step for any language model, up to modern large language models (LLMs).
...
The attention mechanism (see [Architectures: CNNs, RNNs, and Transformers](/?c=ia&s=fondamentaux-du-deep-learning&p=architectures-cnn-rnn-transformers)) lets each token "look at" the other tokens in the sequence to adjust its own representation based on context:
...
An **LLM** (*Large Language Model*) is, in its simplest form, a [Transformer](/?c=ia&s=fondamentaux-du-deep-learning&p=architectures-cnn-rnn-transformers) model trained on huge amounts of text, with a remarkably simple training objective: **predict the next word (or token)**, given everything that came before.
...
See also [Architectures: CNNs, RNNs, and Transformers](/?c=ia&s=fondamentaux-du-deep-learning&p=architectures-cnn-rnn-transformers) (the underlying attention mechanism), [Deep Learning with PyTorch](/?c=ia&s=fondamentaux-du-deep-learning&p=deep-learning-pytorch) (how such a model is concretely trained, at a much smaller scale in that chapter's examples) and [Prompt Engineering](/?c=ia&s=nlp-llm&p=prompt-engineering) (how to concretely phrase a good prompt).
