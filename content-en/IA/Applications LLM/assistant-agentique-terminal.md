...
The previous chapters cover the building blocks of a modern LLM assistant separately: [tools and the agent loop](/?c=ia&s=nlp-llm&p=agents), [RAG](/?c=ia&s=nlp-llm&p=rag), [prompt engineering](/?c=ia&s=nlp-llm&p=prompt-engineering), and [production limitations](/?c=ia&s=nlp-llm&p=llm-en-production). This chapter does not repeat that information; instead, it brings together what is still missing to understand how an assistant that works in a terminal (capable of reading and modifying files, executing commands, and searching the web) actually functions from one turn to the next. Claude on the command line serves here as a concrete example, but nothing is specific to a particular provider: every mechanism described is publicly documented and can be found, sometimes under different names, in most current agent-based assistants.

## Pure generation vs. actual retrieved data

Without a tool, an LLM simply **generates plausible text** based on what it has learned during training (see its definition in [NLP and LLM](/?c=ia&s=nlp-llm&p=nlp-et-llm)): it has no way of consulting any external sources. Two prompts that, on the surface, produce the same kind of response are actually very different:
...
| "What is the current version number of library X?" | Without a tool to verify it, the model produces a response **that appears** just as plausible but may be false: a hallucination (see [LLM in Production](/?c=ia&s=nlp-llm&p=llm-en-production)) | Unreliable without verification |
...
This mechanism should not be confused with the [*chain-of-thought* in prompt engineering](/?c=ia&s=nlp-llm&p=prompt-engineering): in that context, detailed reasoning is a **prompt technique** explicitly requested by the user in their question. Extended internal reasoning, on the other hand, is a **distinct and native generation phase** that exists independently of any prompt instruction regarding it:
...
The same warning as for the prompted chain-of-thought applies, and even more so: a displayed or summarized line of reasoning does not guarantee that it accurately corresponds to the internal mechanism that actually produced the response (see [this pitfall detailed in prompt engineering](/?c=ia&s=nlp-llm&p=prompt-engineering)).

## Specific categories of tools

The [Agents](/?c=ia&s=nlp-llm&p=agents) chapter presents the generic mechanism of function calling using a single example (the weather). In practice, an assistant that works with code or information relies on recurring categories of tools, each with its own trade-offs.
...
The [RAG](/?c=ia&s=nlp-llm&p=rag) queries a database **that is pre-indexed in advance** and remains static between reindexing cycles. In contrast, a real-time web search tool sends a query **at the exact moment the request is made**, without any prior indexing step:

| | RAG | Live Web Search |
|---|---|---|
| Queried database | A pre-built vector index (see [RAG](/?c=ia&s=nlp-llm&p=rag)) | The web as it exists at the time of the query |
...
> **Pitfall:** A loop without an explicit termination condition carries the same risk of an unbounded loop as a classic agent loop (see [Agents](/?c=ia&s=nlp-llm&p=agents)), except that here the loop runs for a single task of questionable quality, not due to a lack of information.
>
...
A call to an LLM typically recalculates the entire prompt at each turn, including tokens already sent in the previous turn (see tokenization in [NLP and LLM](/?c=ia&s=nlp-llm&p=nlp-et-llm)). However, a large portion of an agentic prompt remains the same from one turn to the next within the same session: the system instructions, the list of available tools, and the beginning of the history. The prompt cache reuses the calculation already performed on this common prefix rather than starting from scratch every turn: a practical application of the principle of [never recalculating a result if nothing has changed since then](/?c=performance&p=eviter-le-recalcul-redondant).
...
The [context window](/?c=ia&s=nlp-llm&p=llm-en-production) remains fixed regardless of the model. During a long agentic session, the full history grows with each turn and eventually approaches this limit. Compaction summarizes older turns into a shorter summary before they are removed from the prompt, rather than silently truncating them: the pitfall already noted for the context window in [LLM in production](/?c=ia&s=nlp-llm&p=llm-en-production).
...
The chapter [NLP and LLM](/?c=ia&s=nlp-llm&p=nlp-et-llm) makes a general distinction between fine-tuning (re-training) and prompting (making no changes). A modern conversational AI actually goes through several distinct fine-tuning stages, each of which is publicly documented by the major providers:
