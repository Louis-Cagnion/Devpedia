...
`nn.Linear(inputs, outputs)` automatically creates the corresponding weights and biases (see [Neural Networks: The Fundamentals](/?c=ia&p=reseaux-de-neurones)); `forward()` describes the data's path through the layers, exactly like the "forward pass" detailed by hand in that same chapter.
...
See also [Architectures: CNNs, RNNs, and Transformers](/?c=ia&p=architectures-cnn-rnn-transformers): PyTorch provides ready-made layers for each (`nn.Conv2d`, `nn.LSTM`, `nn.TransformerEncoder`...), built on top of the same basic building blocks seen here.
