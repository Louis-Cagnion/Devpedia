...
**OCaml** is a chance to see a different style, the **functional** style: programs are built by assembling functions and evaluating expressions, rather than chaining statements that modify a state. It's not some exotic lab language — OCaml compiles to native code as fast as C, and is used in production in fields that especially value reliability: finance (Jane Street made it their main language), formal verification (the Coq proof assistant is written in OCaml), and static code analysis.
