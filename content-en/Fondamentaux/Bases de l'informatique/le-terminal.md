...
You type: ls
      │
      ▼
Terminal (the window)     →  passes along the typed text
      │
      ▼
Shell (the interpreter)   →  understands "ls", asks the system for the list of files
      │
      ▼
...
Once opened, the terminal displays a line ending with a symbol (`>`, `$`, `%`...) followed by a blinking cursor: this is the **prompt**. It waits for you to type something; nothing runs until you press `Enter`.
...
│  └───── option: shows details (size, date...)
└──────── command: lists the contents of a folder
```

The exact names of commands vary from one shell to another (`ls` in Bash becomes `Get-ChildItem` in PowerShell); this is the subject of the [Bash](/?c=shells&s=bash&p=bash) and [PowerShell](/?c=shells&s=powershell&p=powershell) chapters, not this one: here, only the general structure (command, options, arguments) matters.
...
| | |
|---|---|
| **Key Points** | The **terminal** displays and transmits the text you type; the **shell** interprets and actually executes it. A **command** consists of a name, options (`-x`), and arguments. Nothing is executed before you press `Enter`. |
