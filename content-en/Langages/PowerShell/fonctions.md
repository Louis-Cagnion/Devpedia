...
Unlike [Bash](/?c=shells&s=bash&p=bash), where a function receives its arguments exactly like a script (`$1`, `$2`, with no name), a PowerShell function declares real **named, typed parameters** via `param()`, like [PHP](/?c=langages-de-programmation&s=php&p=conditions) or [C](/?c=langages-de-programmation&s=c&p=conditions).
