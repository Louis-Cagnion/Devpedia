...
Compared to a `switch` (see the [Conditions](/?c=langages-de-programmation&s=c&p=conditions) chapter, C section), the difference isn't just cosmetic: each branch **extracts** `radius`, or `width` and `height`, directly, with no manual field access (`shape.radius`) or prior type check.
