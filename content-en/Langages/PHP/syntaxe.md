...
> **Note:** `<?= $user ?>` is a shorthand for `<?php echo $user; ?>`; this allows you to use PHP variables in HTML. Whenever the displayed variable could come from user input (a username, for example), you must enclose it in `htmlspecialchars()` as shown above (see [Securing your data](/?c=langages-de-programmation&s=php&p=securite) for the XSS vulnerability this prevents).
