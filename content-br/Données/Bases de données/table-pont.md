...
```sql
-- armadilha: essa consulta conta a venda 1 duas vezes (uma por promocao), logo R$200 em vez de
-- R$100
SELECT SUM(f.valor)
FROM fato_vendas f
...
CREATE TABLE ponte_vendas_promocoes (
    id_venda     INT,
    id_promocao  INT,
    -- parte do valor atribuida a essa promocao (soma = 1 por venda)
    peso         DECIMAL(4, 2)
);

-- com a ponderacao, a soma volta a ficar correta: R$100 divididos em R$50 + R$50, nao R$100 +
-- R$100
