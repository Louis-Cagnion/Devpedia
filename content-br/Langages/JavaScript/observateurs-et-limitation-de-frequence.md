...
| | |
|---|---|
| **Para lembrar** | O `IntersectionObserver` detecta a visibilidade de um elemento sem recálculo manual no scroll (scroll infinito). O `MutationObserver` reage a uma mudança do DOM sem sondar em loop, mas não vê nem propriedades JS atribuídas diretamente nem mudanças fora do DOM. Debounce espera uma pausa antes de executar; throttle executa no máximo uma vez por intervalo. |
| **Ferramentas utilizáveis** | `IntersectionObserver` (`rootMargin`, `isIntersecting`), `MutationObserver` (`childList`/`subtree`/`attributes`), um debounce/throttle caseiro via `setTimeout`. |
...
| **Boas práticas** | `rootMargin` para pré-carregar antes de o elemento estar realmente visível. `subtree: true` assim que a mudança puder ocorrer em qualquer profundidade. Escolher debounce para uma ação final única depois de uma rajada, throttle para um teto regular durante a rajada. |
