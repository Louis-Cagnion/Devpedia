...
| | |
|---|---|
| **Para lembrar** | Um programa nunca vai mais rápido do que seu recurso mais restrito. Paralelizar em alvos independentes é um ganho gratuito; paralelizar em um mesmo alvo concentra a carga em vez de distribuí-la. |
| **Ferramentas utilizáveis** | Um worker por alvo independente, verificação explícita dos códigos de retorno e do volume de resultados obtido. |
| **Armadilhas a evitar** | Adicionar workers além da restrição real (degrada o desempenho); supor que um worker que falha silenciosamente fará o programa principal falhar. |
| **Boas práticas** | Identificar o recurso limitante antes de paralelizar; espalhar o trabalho no tempo em vez de paralelizar quando a restrição é uma cota e a latência importa pouco. |
