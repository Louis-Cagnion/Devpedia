...
```python
for fruta in reversed(frutas):            # cereza, platano, manzana -> VALORES al revés
    print(fruta)

for i in range(len(frutas) - 1, -1, -1):  # 2, 1, 0                  -> INDICES en orden decreciente
    print(frutas[i])
```

`range(len(x) - 1, -1, -1)` (paso de -1) genera los índices de una secuencia en orden decreciente: útil cuando se necesita el propio índice, no solo el valor. Si solo importa el valor, `reversed(x)` es el modismo más legible a preferir.
...
## `max()` / `min()`: encontrar el mayor o el menor
...
max(anchos)         # 45
min(anchos)         # 3
max(12, 45, 3, 28)  # 45 -> también funciona con valores separados, no solo con un iterable

max([], default=0)  # 0 -> evita un ValueError si el iterable está vacío
```

`max(iterable)`/`min(iterable)` aceptan varios valores separados o un único iterable; sin el parámetro `default=`, cualquiera de los dos lanza un `ValueError` sobre un iterable vacío, algo indispensable en cuanto la colección puede legítimamente estar vacía (ej. el ancho máximo de una lista de líneas que puede no contener ninguna celda).
...
| **Herramientas utilizables** | `enumerate()`, `zip()`, `any()`/`all()`, `reversed()` para recorrer al revés, la cláusula `else` de un bucle (ejecutada si no hubo `break`). |
