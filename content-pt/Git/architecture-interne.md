...
Cada dado armazenado pelo Git (o conteúdo de um ficheiro, uma estrutura de pastas, um commit...) é guardado sob a forma de um **objeto**, identificado exclusivamente pelo hash SHA-1 do seu próprio conteúdo:
...
Concretamente, cada objeto é comprimido (zlib, um algoritmo de compressão sem perdas) e armazenado n`.git/objects/`, num caminho derivado do seu hash: os dois primeiros caracteres hexadecimais formam uma subpasta, os 38 restantes constituem o nome do ficheiro (`.git/objects/c6/b7f4a2...`). Trata-se, nem mais nem menos, de uma **tabela de hash** (ver capítulo dedicado, secção C) armazenada diretamente no sistema de ficheiros — a subpasta desempenha o papel de um compartimento (*bucket*).

> **Consequência direta:** dois ficheiros com conteúdo estritamente idêntico produzem o **mesmo** hash e, portanto, o **mesmo** objeto armazenado uma única vez — uma deduplicação automática e gratuita, propriedade inerente ao modelo, e não uma otimização adicionada posteriormente.

## Os quatro tipos de objetos

| Tipo | Conteúdo |
|---|---|
| **blob** | O conteúdo bruto de um ficheiro — apenas os bytes, sem nome de ficheiro nem metadados |
| **tree** | Uma lista de entradas (modo, tipo, nome, hash) — representa uma pasta, sendo que cada entrada aponta para um blob (ficheiro) ou para outra árvore (subpasta) |
...
> **Nota:** um blob não conhece o seu próprio nome de ficheiro — é o `tree` que contém a associação «este nome de ficheiro corresponde a este hash de blob». É por isso que renomear um ficheiro sem alterar o seu conteúdo não cria nenhum blob novo: apenas o `tree` (e, consequentemente, o commit) é alterado.

## O que faz realmente `git add` e, posteriormente, `git commit`

1. `git add ficheiro.txt`: calcula o SHA-1 do conteúdo do ficheiro, comprime-o, grava-o como um objeto **blob** em `.git/objects/` e regista uma entrada no índice (`.git/índice`, o nome verdadeiro do ficheiro na área de staging) associando o caminho do ficheiro a esse hash do blob.
...
## As referências: simples ficheiros de texto
...
Um ramo não é, **literalmente, mais do que** um ficheiro que contém um hash de um commit. O comando «`git branch nouvelle`» cria simplesmente um novo ficheiro em «`.git/refs/heads/`», copiado a partir do commit atual.
...
1. **Um armazenamento chave-valor endereçado por conteúdo**: uma função de hash (SHA-1, ou mais simples para um protótipo) + compressão + um sistema de ficheiros ou uma tabela de hash para armazenar cada objeto sob a sua própria chave.
