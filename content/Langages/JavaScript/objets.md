...
obj.nom;     // 'Jean'
obj['nom'];  // 'Jean', équivalent à obj.nom

obj.ville = 'Paris';  // ajout d'une propriété
obj.age = 26;         // modification d'une propriété

delete obj.age;                // suppression d'une propriété
```
...
| Méthode | Effet |
|---|---|
| `Object.keys(obj)` | Tableau contenant uniquement les clés |
| `Object.values(obj)` | Tableau contenant uniquement les valeurs |
| `Object.entries(obj)` | Tableau de paires `[clé, valeur]`, utile pour parcourir avec une boucle ou `forEach` |
| `Object.assign(cible, ...sources)` | Copie les propriétés des objets source dans l'objet cible, renvoie la cible ; sert souvent à fusionner ou copier |
| `Object.freeze(obj)` | Empêche toute modification (ajout, suppression, changement) : ignorée silencieusement, ou erreur en mode strict |
| `Object.fromEntries(paires)` | Inverse d'`Object.entries` : transforme un tableau de paires `[clé, valeur]` en objet |
...
const { nom: prenom } = obj; // renomme la variable lors du destructuring -> prenom = 'Jean'
```

Le **spread** (`...`) permet de "déplier" un objet, ce qui est utile pour le copier ou en fusionner plusieurs entre eux.

```javascript
const copie = { ...obj };                   // copie indépendante de obj
