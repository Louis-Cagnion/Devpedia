...
| **Outils utilisables** | `:root` pour des variables globales, `var(--nom, valeur-de-secours)`, `element.style.setProperty()` pour les modifier en [JavaScript](/?c=langages-de-programmation&s=javascript&p=javascript), `getComputedStyle().getPropertyValue()` pour les lire. |
