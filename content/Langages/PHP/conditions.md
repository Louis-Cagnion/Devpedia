...
<?php
    ...
    var_dump($a == $b);   // true  -> les deux chaînes sont numériques : 10 == 10
    var_dump($a === $b);  // false -> même type (string) mais contenu littéral différent
    var_dump($a == $c);   // true  -> "10" convertie en entier
    var_dump($a === $c);  // false -> string et int sont des types différents
?>
```
...
Comme pour les autres structures de contrôle, les conditions peuvent s'écrire avec `:` et `end...`, pratique pour mélanger avec du [HTML](/?c=langages-de-balisage&s=html&p=html) :
