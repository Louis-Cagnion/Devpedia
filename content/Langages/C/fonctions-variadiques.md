...
```c
printf("%-10d|\n", 42);     // "42        |" -> '-' : justifié à GAUCHE (par défaut : à droite)
printf("%010d\n", 42);      // "0000000042"  -> '0' : complète avec des zéros plutôt que des espaces
printf("%#x\n", 255);       // "0xff"        -> '#' : forme alternative (préfixe 0x/0X pour x/X)
printf("%+d\n", 42);        // "+42"         -> '+' : force l'affichage du signe, même positif

printf("%10d\n", 42);       // "        42" -> largeur MINIMALE : complète avec des espaces si besoin
printf("%.3d\n", 5);        // "005"        -> précision sur un entier : nombre minimal de chiffres

printf("%*d\n", 10, 42);    // équivalent à "%10d" -> '*' : la largeur est lue depuis les arguments, pas écrite en dur
```
