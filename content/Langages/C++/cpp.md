...
| RAII | Une ressource (mémoire, fichier...) est libérée automatiquement quand l'objet qui la détient est détruit, voir [Gestion mémoire et RAII](/?c=langages-de-programmation&s=cpp&p=gestion-memoire-raii), qui limite drastiquement les fuites mémoire possibles en [C](/?c=langages-de-programmation&s=c&p=c) |
...
C++ garde ainsi le contrôle bas niveau du [C](/?c=langages-de-programmation&s=c&p=c) (mémoire, performance, absence de ramasse-miettes) tout en offrant des outils de plus haut niveau pour structurer un projet de grande taille : un compromis qui explique sa présence durable dans les moteurs de jeux et les systèmes embarqués exigeants.
