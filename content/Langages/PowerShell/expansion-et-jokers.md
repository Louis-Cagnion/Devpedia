...
PowerShell reprend l'idée du globbing [Bash](/?c=shells&s=bash&p=bash) (remplacer un motif par la liste réelle de fichiers correspondants), mais sous un nom différent (*wildcards*) et avec des règles légèrement distinctes, en plus d'un opérateur de correspondance de motif réutilisable en dehors des noms de fichiers.
...
> **Note :** comme le globbing [Bash](/?c=shells&s=bash&p=bash), ce n'est **pas** une [regex](/?c=domain-specific-languages-dsl&p=regex) : ces motifs ne sont interprétés de cette façon que par les cmdlets qui l'annoncent explicitement (`Get-ChildItem`, `-like`), pas par PowerShell lui-même à l'échelle de la ligne entière comme le fait [Bash](/?c=shells&s=bash&p=bash) avant d'exécuter quoi que ce soit.

## `-like` : appliquer un wildcard à une chaîne quelconque

Contrairement à [Bash](/?c=shells&s=bash&p=bash), où le globbing ne s'applique qu'aux noms de fichiers réels sur le disque, `-like` applique les mêmes motifs à n'importe quelle chaîne :
...
Équivalent le plus proche de l'expansion d'accolades `{1..5}` de [Bash](/?c=shells&s=bash&p=bash), mais limité aux plages numériques :
...
| | |
|---|---|
| **À retenir** | Les wildcards PowerShell (`*`, `?`, `[]`) ressemblent au globbing [Bash](/?c=shells&s=bash&p=bash), mais ne sont interprétés que par les cmdlets qui l'annoncent explicitement : PowerShell lui-même ne les développe jamais à l'échelle de toute la ligne comme le fait [Bash](/?c=shells&s=bash&p=bash). |
| **Outils utilisables** | `-like` (wildcard sur une chaîne quelconque), l'expansion de plage (`1..5`). |
| **Pièges à éviter** | S'attendre à ce qu'un motif non résolu s'affiche littéralement comme en [Bash](/?c=shells&s=bash&p=bash) : PowerShell résout toujours en une vraie liste, éventuellement vide. |
