...
> **Note :** `::before`/`::after` nécessitent une propriété `content` pour être visibles (même vide, `content: "";`) — très utilisés pour ajouter un élément purement décoratif (icône, flèche...) sans alourdir le HTML d'une balise supplémentaire sans réelle signification sémantique (cf. chapitre sur la sémantique HTML5).
...
| `!important` | Écrase tout le reste (à éviter, cf. chapitre sur la cascade) |

Voir aussi le chapitre sur la cascade, qui détaille précisément l'ordre de résolution entre spécificité, ordre d'écriture et origine de la règle.
