...
> **Note :** `<br>` ne doit pas être utilisé pour créer un espacement visuel entre deux paragraphes : c'est le rôle de [CSS](/?c=langages-de-balisage&s=css&p=css) (`margin`, voir [Le modèle de boîte](/?c=langages-de-balisage&s=css&p=box-model)). Un usage répété de `<br><br>` pour "faire de l'espace" est un signe qu'on utilise HTML pour de la présentation, alors que ce n'est pas sa responsabilité.
...
| | |
|---|---|
| **À retenir** | Le choix d'une balise de texte doit refléter le sens du contenu (titre, paragraphe, liste, emphase), jamais seulement l'apparence visuelle souhaitée : l'apparence relève de [CSS](/?c=langages-de-balisage&s=css&p=css). |
