...
```bash
# liste les remotes configurés (souvent juste "origin")
git remote -v
...
```bash
# envoie les commits de la branche locale "main" vers le remote "origin"
...
```bash
# écrase l'historique distant sans condition, dangereux si quelqu'un d'autre a poussé
# entre-temps
git push --force origin main
# écrase seulement si le remote est encore dans l'état vu lors du dernier fetch
...
```bash
# télécharge les nouveaux commits du remote, SANS toucher au dossier de travail
git fetch origin
...
```bash
# capture toutes les refs (branches, tags, HEAD) dans un seul fichier
...
Voir aussi [GitHub et les plateformes d'hébergement Git](/?c=git&p=github-et-plateformes) pour ce qu'une plateforme comme GitHub ajoute par-dessus un simple remote ([pull requests](/?c=git&p=pull-requests-github), [issues](/?c=git&p=issues-et-projets-github)), et [Résoudre un conflit de fusion](/?c=git&p=resoudre-conflits), fréquemment nécessaire après un `pull` quand plusieurs personnes ont modifié les mêmes lignes.
