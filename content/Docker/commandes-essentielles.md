...
| `-p 8080:80` | Publie le port : le port `80` du conteneur devient accessible sur le port `8080` de l'hôte (cf. chapitre sur les réseaux) |
...
docker logs -f serveur     # suit les logs en direct (equivalent de `tail -f`, cf. chapitre sur le traitement de texte)
```
