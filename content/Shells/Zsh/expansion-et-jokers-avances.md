...
Le globbing de base (`*`, `?`, `[abc]`) fonctionne à l'identique en zsh (voir [Expansion et jokers](/?c=shells&s=bash&p=expansion-et-jokers) en [Bash](/?c=shells&s=bash&p=bash)). Zsh va nettement plus loin une fois le mode étendu activé, avec des motifs que [Bash](/?c=shells&s=bash&p=bash) ne comprend simplement pas.
...
> **Note :** en [Bash](/?c=shells&s=bash&p=bash), ce comportement récursif nécessite `shopt -s globstar` (option équivalente, mais absente par défaut et propre à [Bash](/?c=shells&s=bash&p=bash) 4+) ; en zsh, `**` fonctionne dès que `EXTENDED_GLOB` (ou même sans, `**` seul est actif par défaut dans la plupart des configurations récentes) est actif, sans réglage supplémentaire.
...
> **Note :** ces qualificatifs remplacent, pour beaucoup de cas simples, un `find . -type f` ou un `find . -mtime -7` (voir [Permissions et manipulation de fichiers](/?c=shells&s=bash&p=permissions-et-fichiers) en [Bash](/?c=shells&s=bash&p=bash)), directement dans le motif du glob, sans lancer de commande externe.

## Combiner globbing étendu et guillemets

Comme en [Bash](/?c=shells&s=bash&p=bash), entourer un motif de guillemets désactive son interprétation (voir [Les variables](/?c=shells&s=bash&p=variables) en [Bash](/?c=shells&s=bash&p=bash) pour la logique guillemets simples/doubles) :
...
| | |
|---|---|
| **À retenir** | Le globbing de base fonctionne comme en [Bash](/?c=shells&s=bash&p=bash) ; `EXTENDED_GLOB` débloque des motifs propres à zsh (`**` récursif, négation `^`, qualificatifs entre parenthèses). |
