...
Bash active des comportements optionnels au cas par cas (`shopt -s nom`, `set -o nom`, cf. chapitres Bash concernés). Zsh centralise ça dans un seul mécanisme cohérent : `setopt`/`unsetopt`, avec des dizaines d'options nommées qui changent le comportement du shell.
...
| `EXTENDED_GLOB` | active les motifs de globbing étendus (cf. chapitre suivant) |
...
> **Note :** ces options sont typiquement placées dans `~/.zshrc` (cf. chapitre sur les fichiers de démarrage) pour être actives dans chaque nouveau terminal, exactement comme un `shopt -s` serait placé dans `~/.bashrc`.
