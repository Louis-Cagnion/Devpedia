...
OCaml est **statiquement typé** : chaque expression a un type fixé une fois pour toutes, vérifié avant même l'exécution, comme en C (cf. chapitre [Les variables et types de données](/?c=langages-de-programmation&s=c&p=variables)). Contrairement au C, ce type n'a presque jamais besoin d'être écrit explicitement :
...
Le second exemple illustre le **polymorphisme paramétrique** : `'a` signifie "un type quelconque, à déterminer selon l'appel", la même idée qu'un template C++ (cf. chapitre [Les templates](/?c=langages-de-programmation&s=cpp&p=templates)), mais résolue automatiquement par inférence plutôt que déclarée explicitement à chaque utilisation (`template<typename T>`).
