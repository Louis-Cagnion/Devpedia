...
Un [réseau de neurones](/?c=ia&s=fondamentaux-du-deep-learning&p=reseaux-de-neurones) commence avec des poids **aléatoires** : ses prédictions initiales n'ont donc aucun sens. **L'entraînement** est le processus qui ajuste progressivement ces poids pour que les prédictions se rapprochent des bonnes réponses, à partir d'exemples.
...
> **Note :** ce n'est pas une opération à recalculer à la main pour utiliser un framework comme [PyTorch](/?c=ia&s=fondamentaux-du-deep-learning&p=deep-learning-pytorch) : `autograd` (différenciation automatique) effectue ce calcul automatiquement. Comprendre le **principe** (propager le gradient en arrière, couche par couche, via la règle de dérivation en chaîne) suffit pour raisonner sur pourquoi certains problèmes d'entraînement surviennent (ex. le "vanishing gradient", voir [Architectures : CNN, RNN et Transformers](/?c=ia&s=fondamentaux-du-deep-learning&p=architectures-cnn-rnn-transformers)).
...
Voir aussi [Deep learning avec PyTorch](/?c=ia&s=fondamentaux-du-deep-learning&p=deep-learning-pytorch), qui automatise entièrement cette boucle d'entraînement (`loss.backward()`, `optimizer.step()`).
