...
Un chatbot plus riche ajoute un appel [RAG](/?c=ia&s=nlp-llm&p=rag) avant l'appel au modèle (chercher un contexte pertinent à injecter) et/ou des outils au sens des [agents](/?c=ia&s=nlp-llm&p=agents) (consulter une commande, une base de stock, envoyer un email), mais les trois éléments ci-dessus restent le socle, avec ou sans ces extensions.
...
> **Piège :** traiter le system prompt comme une barrière de sécurité. Un utilisateur déterminé peut tenter de le faire ignorer par le modèle (voir la [prompt injection](/?c=ia&s=nlp-llm&p=prompt-injection)) : un garde-fou censé être absolu (ne jamais confirmer un virement, ne jamais donner un diagnostic médical) qui ne reposerait que sur une instruction texte peut être contourné.
>
...
**La gestion de l'historique a une limite physique.** La fenêtre de contexte est bornée (voir [LLM en production](/?c=ia&s=nlp-llm&p=llm-en-production)) : une conversation longue finit par ne plus tenir dans un seul prompt. Deux stratégies, souvent combinées :
...
- **L'injection différée.** Une instruction malveillante (voir la [prompt injection](/?c=ia&s=nlp-llm&p=prompt-injection)) n'a pas besoin d'arriver dans le premier message : elle peut être glissée plusieurs tours plus tard, une fois la conversation "installée", en espérant que le modèle lui accorde plus de poids qu'au system prompt initial.
- **L'absence de porte de sortie.** Un chatbot qui ne sait pas dire *"je ne suis pas sûr, voici comment contacter un humain"* pousse l'utilisateur à insister jusqu'à obtenir une réponse, potentiellement une hallucination (voir [LLM en production](/?c=ia&s=nlp-llm&p=llm-en-production)), plutôt qu'un renvoi honnête vers une escalade humaine. Prévoir explicitement ce mécanisme de bascule fait partie de la conception, pas seulement du filet de sécurité.
- **La transparence n'est pas optionnelle.** Dans l'Union européenne, un chatbot relève typiquement du risque "limité" de l'[AI Act](/?c=ia&s=production-et-gouvernance&p=reglementation-europeenne-ia) : l'utilisateur doit toujours pouvoir savoir qu'il interagit avec une IA, pas un humain : une obligation légale, pas seulement une bonne pratique d'UX.
...
**Le streaming améliore la latence perçue, pas la latence réelle.** Un modèle produit sa réponse token par token (voir [LLM en production](/?c=ia&s=nlp-llm&p=llm-en-production)) ; l'afficher au fur et à mesure plutôt que d'attendre la réponse complète ne raccourcit pas le temps de calcul total, mais évite à l'utilisateur de fixer un écran vide pendant plusieurs secondes.
...
>
> **Bonne pratique :** mettre en place un rate limiting par utilisateur (voir les garde-fous de coût dans [Monitoring et gestion opérationnelle d'un LLM](/?c=ia&s=production-et-gouvernance&p=gestion-dun-llm)).
...
>
> **Bonne pratique :** cloisonner strictement l'historique et tout contexte injecté par client (voir [Gouvernance des données](/?c=ia&s=production-et-gouvernance&p=gouvernance-des-donnees) pour le contrôle d'accès aux documents sous-jacents).
