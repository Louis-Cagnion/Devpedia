...
- **Actions irréversibles décidées par un système faillible** : un agent qui peut envoyer un email ou modifier une base de données peut aussi le faire à tort, sur la foi d'un raisonnement erroné. La confirmation humaine avant toute action à conséquence réelle (financière, destructrice, visible publiquement) n'est pas une prudence excessive, c'est la contrepartie du non-déterminisme du modèle qui décide.
