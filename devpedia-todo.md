...
## 2. Surlignage désynchronisé de la vitesse de lecture (×1.25/×1.5/×2) : corrigé, à confirmer
...
## 3. Voix bloquée sur le tableau de l'accueil : corrigé, à confirmer
...
## 4. "Paragraphe suivant/précédent" bloquait en entrant dans un tableau : corrigé, à confirmer
...
## 5. Reprise après verrouillage écran : audio, surlignage et scroll perdent la position réelle
...
## 6. Contrôles Bluetooth/écran verrouillé : suivant/précédent change de paragraphe, artwork ajoutée
...
## 7. Auto-advance vers le chapitre suivant : le délai de 5s ne s'écoulait pas si verrouillé
...
## 8. Pause de lecture sur les parenthèses
...
## 9. Tester l'audio prégénéré C/C++/PHP/SQL/Git (4 langues)
...
## 10. Fond étoilé des pages chapitre invisible sur mobile (iOS 16.7.16) : deux correctifs déjà tentés, sans effet
...
- PowerShell : `Power Shell` puis `Power-Shell` (essai de Louis, "pour voir") : version actuelle, pas encore confirmée. Zsh (`Z-S-H`, tiret) : mis à jour préventivement, pas encore confirmé spécifiquement.
- Git : `Guite` (raté) puis `G I T` (rejeté à l'oreille) puis `Gui te` puis `Gui-t` (essai de Louis, "pour voir") : version actuelle, pas encore confirmée.
...
- Reste à Louis : confirmer à l'oreille sur l'accueil PowerShell (`Power-Shell`), Git (`Gui-t`), Zsh, Blockchain.
...
- **Validation de la table de prononciation TTS** (`js/reader-pronunciation.js`), chapitre par chapitre par Louis en écoute directe : reste tout hors C/C++/SQL (déjà validés le 2026-08-15) ; Git/PHP retirés de cette liste suite au point 11 ci-dessus (leur validation du 15/08 ne couvrait pas ces prononciations précises).
