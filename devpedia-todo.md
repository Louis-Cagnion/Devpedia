...
2. Une fois (1) en place, Louis a précisé le vrai symptôme : c'est systématiquement le premier mot de **chaque segment** qui se coupe puis reprend depuis le début. Cause trouvée : `speakNextViaAudio()` appelait `audioEl.play()` juste après avoir réglé `audioEl.currentTime`, sans attendre que ce seek (asynchrone) soit terminé. **Premier correctif (23/08/2026, insuffisant)** : attendre l'événement `"seeked"` avant de jouer. Louis a fourni un enregistrement d'écran (avec l'overlay de debug visible) confirmant le vrai symptôme : un couple `seeking`/`seeked` sans aucun `play` derrière, lecture bloquée en silence jusqu'à ce que le segment suivant la "rattrape" plusieurs secondes après -- l'écouteur `"seeked"` ratait l'événement réel (course avec un seek déjà en cours). **Deuxième correctif (23/08/2026, à confirmer)** : un filet de sécurité (`setTimeout` 300ms) garantit maintenant que `play()` est toujours appelé, `"seeked"` reçu ou non. `audioEl.play()` catch aussi son rejet désormais (loggé via `reader-debug.js`) au lieu de planter silencieusement.
