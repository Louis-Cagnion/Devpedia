...
Signalé par Louis (29/08/2026), en plusieurs retours successifs (détail des 2ᵉ/3ᵉ retouches dans `journal-de-bord.md`) : PHP lu en un mot, PowerShell lu "powshell", Git lu "gi" puis "yite" (le respelling "Guite" a raté), UI/UX lu "uzi/uzx" puis "usi/utix" (l'espace entre lettres ne suffisait pas), Zsh lu comme un mot tenté. État actuel :
- Sigles en MAJUSCULES (PHP, HTML, UI/UX, CI/CD...) : mécanisme générique `spellOutAcronymsFr()`, virgule entre chaque lettre.
- PowerShell ("Power Shell"), Git (abandon du respelling, épelé `G I T` comme un sigle malgré la casse), Zsh (`Z S H`) : entrées dédiées, casse mixte non couverte par le mécanisme générique.
- Reste à Louis : confirmer à l'oreille ces 3 dernières retouches (UI/UX, Git, Zsh) et PowerShell -- ce dernier peut encore sonner faux si testé sur un chapitre PowerShell/Git/PHP/Zsh dont l'audio n'a pas encore été régénéré (seuls ces 4 contextes + l'accueil l'ont été aujourd'hui, pas le reste du site).
