...
> Prochaine tâche : traiter les notions manquantes 3 à 5 (issues de la revue `/review` PDF_parser), ou attendre le retour de Louis sur le point 1. Régénération audio complète seulement une fois tout ajouté.

> Restent : un test navigateur en attente de Louis pour continuer l'investigation (point 1). L'audio (FR et autres langues) sera régénéré en un seul passage une fois tous les chapitres ajoutés, pas point par point.
...
## 2. Notion manquante : `os.environ` (Python, variables d'environnement)
...
## 3. Notion manquante : compréhension imbriquée (double `for`) pour aplatir une liste de listes
...
## 4. Notion manquante : `frozenset` (Python, variante immuable de `set`)
