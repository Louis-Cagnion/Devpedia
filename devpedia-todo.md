...
- Réécriture des chapitres ES **déjà existants** mais legacy (décidé le 2026-08-14, comme pour BR, sans agents) : audit initial sur 118 fichiers a trouvé 89 sans résumé final, 29 tronqués (jusqu'à 54% du FR actuel), 48 avec tirets cadratins.
  ...
  - Shells/Bash : 15/15 faits, catégorie terminée. `commandes-de-base.md` et `automatisation-cron.md` étaient deux chapitres entièrement absents côté ES (contenu neuf) : créés. Plusieurs sections entières manquantes retrouvées dans des fichiers par ailleurs déjà présents (ex : toute la section "sous-shells" et "comment le noyau reconnaît un script" absentes d'`architecture-dun-shell.md`, toute la section CSRF/panorama des attaques déjà notée pour PHP a son équivalent ici avec la section "enchaîner des commandes" absente de `redirections-et-pipes.md`). Bug plus grave que d'habitude : dans `variables-denvironnement.md`, la commande `source` avait été traduite littéralement en `fuente` dans les blocs de code, ce qui aurait cassé l'exemple si un lecteur l'avait copié-collé.
  - Pas commencé : Python (15 fichiers propres au sujet, voir écart de structure ci-dessous).
