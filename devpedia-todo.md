...
## 1. Boutons de lecture désynchronisés de la voix (play() rejeté) : corrigé, à confirmer
Signalé par Louis (29/08/2026), après plusieurs correctifs ponctuels de la journée : "les boutons ne sont absolument pas synchronisés avec la voix". Audit ciblé de `js/reader.js` (détail dans `journal-de-bord.md`) : `audioEl.play()` rejeté par la politique autoplay du navigateur laissait `isPlaying` bloqué à `true` pour toujours (bouton "Pause" affiché, aucun son). Corrigé : l'état retombe sur "en pause" si `play()` échoue, et le bouton bascule immédiatement au tap plutôt que d'attendre en silence le chargement de l'audio pré-généré.
- Reste à Louis : confirmer sur plusieurs pages/langues que les boutons play/pause/reprendre reflètent bien ce qui joue réellement, y compris juste après avoir changé de page ou de chapitre.
