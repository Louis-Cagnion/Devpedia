...
Piste restante, désormais nécessaire (les fixes passifs — `<audio>`, `mediaSession`, mode standalone — sont épuisés sans résultat). Louis n'a pas de Mac (donc pas de Web Inspector distant classique) : log de diagnostic embarqué dans le site à la place (`js/reader-debug.js`), écrit dans `localStorage` (survit même si iOS tue et recharge la page en arrière-plan) et affiché en overlay sur l'écran, sous la navbar. Trace les événements `audioEl` (play/pause/stalled/suspend/waiting/ended/error, self vs. externe), les handlers `mediaSession` (Bluetooth), et le cycle de vie de la page (visibilitychange, pageshow/pagehide + un id de boot aléatoire pour repérer un redémarrage complet du processus).

**Premier essai (22/08/2026) sans lecture lancée avant verrouillage : à refaire.** Le stockage d'une app ajoutée à l'écran d'accueil n'est en plus pas partagé avec l'onglet Safari (confirmé par Louis), donc le flag ne passait pas par `?debug=1` dans l'URL — remplacé par un geste indépendant de toute URL : **5 taps sur le logo "Devpedia" dans la navbar (moins de 2,5s) active/désactive le log et recharge la page**, fonctionne aussi bien dans Safari que depuis l'icône installée.
- Reste à Louis : depuis l'icône sur l'écran d'accueil, taper 5 fois sur le logo pour activer le log, presser "Écouter cette page" pour lancer la lecture, verrouiller, attendre, déverrouiller, et lire l'overlay affiché sous la navbar.
