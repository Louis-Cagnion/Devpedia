...
## 2. Notion manquante : l'API `re` de Python et les groupes nommés (`(?P<nom>...)`)
- Repéré dans le projet PDF_parser (`normalization.py`, revue `/review`) : `Langages/Domain-specific Languages (DSL)/regex.md` couvre la syntaxe des regex en général (motifs, groupes capturants numérotés `match[1]`) mais pas l'API Python spécifique (`re.compile()`, méthode `.match()` sur un `Pattern` précompilé, objet `Match` et accès `match.group(0)`/`match["nom"]`), ni les groupes nommés `(?P<nom>...)`.
- Angle à couvrir : `re.compile(motif)` précompile un motif en objet `Pattern` réutilisable (plus efficace que de le réinterpréter à chaque appel) ; `pattern.match(texte)` renvoie un objet `Match` (ou `None` si pas de correspondance) ; un groupe nommé `(?P<nom>...)` permet de récupérer un fragment capturé par son nom (`match["nom"]` ou `match.group("nom")`) plutôt que par sa position (`match[1]`), bien plus lisible dès qu'un motif a plusieurs groupes (ex. jour/mois/année d'une date).
- Rubrique cible probable : `Langages/Python` (nouveau chapitre, ex. `regex-en-python.md`), en renvoyant vers le chapitre DSL existant pour la syntaxe générale des motifs.
