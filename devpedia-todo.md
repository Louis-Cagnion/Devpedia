...
- Traduction de la catégorie `Authentification` (créée le 2026-08-15, 4 subjects/7 chapitres) et du chapitre `reduire-la-variance-des-reponses.md` (IA > NLP et LLM) — **en cours**, reprendre ici :
  ...
  - EN : **fait** (contenu + `categoryLabels`/`subjectLabels` dans `ui-strings.json`, `struct-en.json` régénéré, 0 lien cassé, comptes de fichiers alignés 12/12/12 avec FR/ES).
  - BR : **pas commencé**, ni le chapitre ni la catégorie. Reprendre sur le même modèle que ES/EN : contenu de la catégorie, `categoryLabels`/`subjectLabels` BR dans `ui-strings.json`, régénérer `struct-br.json` (script : `buildStruct`/`writeStruct`/`validateInternalLinks` de `scripts/generate-struct.js`, importés depuis un `node -e` pointant sur `content-br`), revalider les liens, comparer les comptes de fichiers avec FR/ES/EN.
