...
- Contrôles de lecture à étendre (demandé par Louis le 2026-08-15) :
  - Mémoriser le mot exact où la lecture s'est arrêtée (pas seulement le paragraphe, cf. `lastSpokenIndex`/`replayParagraph()` actuels dans `js/reader.js`), pour reprendre pile à cet endroit plutôt que de tout relire depuis le début du paragraphe. Plus simple qu'avant : `activeWordIndex` est maintenant renseigné en continu (estimation ou `boundary`, cf. `js/reader.js`), pas seulement quand `boundary` se déclenche.
