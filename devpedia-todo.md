...
## 8. Compléments ciblés à `agents.md` / `assistant-agentique-terminal.md` (pas une nouvelle section)
**Correction du 21/08/2026** : cette entrée s'appelait auparavant « Section outils et agents IA », proposée à tort en supposant que le chapitre `agents` n'existait pas encore. En réalité `content/IA/NLP et LLM/agents.md` **et** `content/IA/Applications LLM/assistant-agentique-terminal.md` existent déjà et couvrent déjà, en détail et avec rigueur, la quasi-totalité de ce qui avait été planifié : function calling, JSON, boucle ReAct, piège/bonne pratique sur la validation des arguments avant exécution, patrons multi-agents (pipeline, orchestrateur/travailleurs, blackboard, évaluateur/optimiseur), diff/patch vs réécriture complète, cache de prompt, compaction de contexte, et surtout le post-entraînement détaillé (pré-entraînement → SFT → RLHF → Constitutional AI), plus précis que ce qui était prévu ici. Créer une nouvelle section aurait dupliqué ce contenu, contrairement à la règle de `prompt.md` sur les redirections plutôt que les redites. Ne PAS écrire les 3 chapitres qui suivaient dans la version précédente de cette entrée.

Ce qui reste réellement non couvert (3 compléments ciblés, à intégrer dans les chapitres existants, pas en nouveaux chapitres) :
1. **Construction du contexte d'un sous-agent** : `agents.md` couvre les patrons de coordination multi-agents (pipeline, orchestrateur/travailleurs, blackboard, évaluateur/optimiseur) mais ne précise nulle part qu'un sous-agent démarre par défaut avec un contexte **vide** (sauf mécanisme explicite de copie d'historique complet) — c'est le prompt rédigé par l'agent appelant qui constitue tout le contexte transmis, pas un héritage automatique. À ajouter dans le tableau des patrons de `agents.md`, ou en note sous celui-ci.
2. **JSON Schema comme système de types** : `agents.md` montre déjà un exemple JSON de function calling et signale le piège de ne pas valider les arguments avant exécution — mais ne présente jamais JSON Schema comme un vrai système de types (`type`/`items`/`properties`/`required`/`enum` ≈ équivalents de `int`/`string`/`tableau`/`struct` en langage classique), ni d'exemples concrets multi-langages (Python/Pydantic, Node.js/Ajv, Go + tags `json`) montrant comment un langage typé classique reçoit/valide cet appel. Un ajout court à la section function calling de `agents.md` plutôt qu'un chapitre séparé — vérifier au moment de l'écriture qu'un langage cité (Go notamment) a bien un chapitre existant à lier, sinon l'introduire en une phrase.
3. **Modèle unique vs modèles historiquement séparés par modalité** : `assistant-agentique-terminal.md` détaille déjà les étapes de post-entraînement (SFT/RLHF/Constitutional AI), mais ne répond pas explicitement à la question « un seul modèle qui fait tout, ou un modèle séparé spécialisé code/outils ? ». Angle étroit, à ajouter en une ou deux phrases dans ce chapitre plutôt qu'en développement séparé — mentionner en contraste les anciens modèles « code-only » (type Codex historique) vs l'unification actuelle, nom propre à introduire en une phrase ou lier.
4. **Jargon nommé sans aucun lien (viole la règle 7bis de `prompt.md`)** — trouvé par un audit du 21/08/2026 sur `assistant-agentique-terminal.md`, section « Les étapes du post-entraînement d'un assistant moderne » : **SFT**, **RLHF** et **Constitutional AI** sont chacun nommés et expliqués en détail, mais aucun des trois n'a de lien, contrairement à « Pré-entraînement » juste au-dessus qui renvoie vers `nlp-et-llm`. À corriger en même temps que le point 3 ci-dessus puisque c'est le même chapitre : ajouter un lien externe stable pour RLHF (papier de référence) et Constitutional AI (page Anthropic), aucun chapitre interne ne les couvrant en détail par ailleurs.

Point séparé, priorité plus faible, même défaut ailleurs dans `IA`, sans lien avec `agents`/`assistant-agentique-terminal` : **LSTM** et **GRU** sont mentionnés sans lien dans `Fondamentaux du deep learning/architectures-cnn-rnn-transformers.md` et `Vision et OCR/ocr-classique-vs-deep-learning.md` (brièvement définis en ligne, mais sans destination pour approfondir) — à corriger dans ces deux fichiers, indépendamment du reste de cette entrée.
