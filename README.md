...
- **Fundamentals**: absolute-beginner groundwork the rest of the site leans on, covering computing vocabulary and tooling (*Bases de l'informatique*: files, the terminal, an editor/IDE, keyboard shortcuts), algorithm analysis independent of implementation (*Algorithmes*: Big-O, comparison sorting), the math vocabulary the AI/Data Science chapters need, starting from zero (*Mathématiques*: functions, vectors, matrices, logarithms, probability, derivatives/gradients), and low-level 3D rendering built from scratch (*Graphisme*: raycasting, OBJ parsing, the Phong model, a GLFW/GLAD render loop, mesh editing)
- **Languages**: general-purpose programming languages: C, C++, PHP, Python, JavaScript, OCaml (functional, contrasted with the imperative style of the others); markup languages: HTML, CSS; shells: Bash (incl. cron scheduling), Zsh, PowerShell; and domain-specific languages: Regex, SQL. Data science/AI tooling for Python lives in its own cross-cutting category below, not under Python itself
- **Data**: data end to end, covering how a value is actually represented in memory (*Représentation des données*: integer overflow, IEEE 754 floats, text encodings, memory layout, pseudo-randomness), organizing it once it outgrows a single table (*Bases de données*: star schema, medallion architecture, bridge tables, warehouses vs. data lakes, indexes, ORMs, high-traffic patterns), the Python tooling for data science and classical ML (*Data Science*: NumPy, pandas, Matplotlib, Jupyter, scikit-learn), and extracting information from existing documents (*Traitement de documents*)
- **Security**: the major vulnerability families and secure development practices (*Cybersécurité*: OWASP Top 10, dependency/API/secrets security, WAF, HashiCorp Vault...), everything about a user's identity (authentication vs. authorization, sessions/tokens, OAuth2/OIDC/SSO, MFA), and offensive security (*Sécurité offensive*: low-level program execution, memory corruption, reverse engineering basics, privilege escalation, CTF)
- **Infrastructure & DevOps**: networking basics, data exchange between systems (APIs, HTTP, WebSocket, JSON, the cloud), containers (Docker: images, Dockerfiles, volumes/networking, Compose, security), Kubernetes (core concepts, services/ingress, ConfigMaps/Secrets, autoscaling/rolling updates), server administration, operating system internals (building one step by step), workflow automation (n8n), and CI/CD (the concept, Azure DevOps, pipeline YAML, vs. GitHub Actions)
- **Code Quality, Performance & Tools**: single responsibility and low coupling, avoiding repetition via indexed/dispatch structures, single source of truth (*Qualité et architecture du code*); language-agnostic optimization: profiling, avoiding fixed delays, cutting round trips, parallelism, cache locality/SIMD, never recomputing a result nothing could have changed (*Performance*); and Git, from everyday commands to its internal object model
- **AI**: neural network fundamentals through production LLM systems (gradient descent, CNN/RNN/Transformer architectures, PyTorch, prompt engineering, agents, RAG, monitoring, EU AI Act, chatbots), vision (OCR, document understanding, layout detection), voice synthesis, and game AI by imitation
...
- **Project Management & Organization**: the human side of a software project, covering team roles, a project's lifecycle, writing an actionable ticket (*Organisation en entreprise*), and Agile/Scrum/Kanban methodologies (*Gestion de projet*)
...
Security isn't confined to the Security category alone: it's also woven into whichever language chapter it concretely applies to (PHP's `securite.md` for the web attack taxonomy: CSRF, XSS, SQL injection, MITM, session hijacking...; command injection in Bash; buffer overflows in C; least-privilege database accounts in SQL), each cross-referencing the others rather than repeating the same explanation.

## Translations

`content/` is the French source of truth, translated by hand into `content-<lang>/`. `scripts/apply-variable-glossary.mjs` retrofits `scripts/variable-glossary.json`'s identifier renaming onto translated files with zero API calls, reusing the text-segmentation logic in `scripts/markdown-segmenter.mjs` (which splits a chapter into natural-language vs. code/markdown-syntax pieces so only the former ever gets rewritten). Currently translated (see `structure/languages.json`): English, Spanish, Portuguese (Brazil). The language switcher lives in the navbar (`js/lang.js`).
