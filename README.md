...
- **AI**: neural network fundamentals through production LLM systems — gradient descent, CNN/RNN/Transformer architectures, PyTorch, NLP & LLM basics, prompt engineering, production use cases and limits, agents, RAG, LLM monitoring, data governance for AI systems, EU AI regulation (the AI Act), and chatbots; also vision (classic OCR through Transformer-based document understanding, layout detection, evaluation and production monitoring) and voice synthesis (concatenative through neural TTS, voice cloning and its ethics, evaluation, production)
